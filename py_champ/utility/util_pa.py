...
class BaseSchedulerByTypeFiltered(BaseScheduler):
    ...
    def do_each(self, method, agent_keys=None, shuffle=False, agt_type=None):
        ...
        if agt_type is not None:
            ...
            if agt_type == "Behavior" and any(hasattr(self._agents[k], 'seniority_id') for k in agent_keys):
                agent_keys.sort(key=lambda x: int(getattr(self._agents[x], 'seniority_id', 99999)))
        if shuffle:
            self.model.random.shuffle(agent_keys)
        for agent_key in agent_keys:
            if agent_key in self._agents:
                getattr(self._agents[agent_key], method)()
                if agt_type == "Behavior":
                    seniority_id = getattr(self._agents[agent_key], 'seniority_id', 'No ID')
                    print(f"Seniority ID {seniority_id}, Accumulated pumping: {self.model.accumulated_withdrawal}")
