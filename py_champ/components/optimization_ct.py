...
class Optimization:
    ...
    def setup_ini_model(
        ...
        # Total water bought per year
        water_traded = m.addMVar(
            (n_s, n_c, n_h), vtype="C", name="water_traded(cm)", lb=-inf, ub=inf)
        # # Total water sold per year
        # water_sold = m.addMVar(
        #     (n_s, n_c, n_h), vtype="C", name="water_sold(cm)", lb=0, ub=inf)
        # # Average water bought per split per crop per yr
        # water_bought_per_field = m.addMVar(
        #     (n_s, n_c, n_h), vtype="C", name="water_bought_per_field(cm)", lb=0, ub=inf)
        # Average water sold per split per crop per yr
        water_traded_per_field = m.addMVar(
            (n_s, n_c, n_h), vtype="C", name="water_traded_per_field(cm)", lb=-inf, ub=inf)
        ...
        self.vars_["y"] = y
        self.vars_["e"] = e
        self.vars_["water_traded"] = water_traded
        # self.vars_["water_sold"] = water_sold
        ## Average values over fields
        self.vars_["y_y"] = y_y
        self.vars_["profit"] = profit
        self.vars_["irr_depth_per_field"] = irr_depth_per_field
        self.vars_["water_traded_per_field"] = water_traded_per_field
        # self.vars_["water_sold_per_field"] = water_sold_per_field
    ...
    def setup_constr_field(
        ...
        **kwargs:
        ...
        water_traded = m.addMVar(
            (n_s, n_c, n_h), vtype="C", name=f"{fid}.water_traded(cm)", lb=0, ub=ub_irr
        )
        ...
        # wr_dict = wr_dict
        # if wr_dict["applied_field_ids"] == "all" or fid in wr_dict["applied_field_ids"]:
        #     m.addConstr((water_traded == irr_depth - wr_depth), name=f"c.{fid}.water_traded(cm)")
    ...
    def setup_constr_finance(self, finance_dict):
        ...
        trading_price = finance_dict["trading_price"]
        crop_profit = {
        ...
        water_traded = vars_["water_traded"]
        cost_e = m.addMVar((n_h), vtype="C", name="cost_e(1e4$)", lb=0, ub=inf)
        trading_cost = m.addMVar((n_h), vtype="C", name="cost_e(1e4$)", lb=0, ub=inf)
        ...
        m.addConstr((trading_cost == water_traded * trading_cost), name="c.trading_cost")
        m.addConstr(
            rev
            == gp.quicksum(
                y[i, j, :] * crop_profit[c]
        ...
        vars_["trading_cost"] = trading_cost
    ...
    def setup_constr_wr(
        ...
        n_s = self.n_s
        vars_ = self.vars_
        # water_sold = self.vars_["water_sold"]
        ...
        # Collect water sold over the constrained fields.
        if fids == "all":
            water_traded_sub = vars_[
                "water_traded_per_field"
            ]  # (n_s, n_c, n_h) averaged irrigation for all the fields a farmer owns
        else:
            for i, fid in enumerate(fids):
                if i == 0:
                    water_traded_sub = vars_[fid]["water_traded_per_field"]
                else:
                    water_traded_sub += vars_[fid]["water_traded_per_field"]
            water_traded_sub = water_traded_sub / len(fids)
        ...
        if remaining_tw is not None and remaining_wr is not None:
            m.addConstr((
                ...
                / n_s) + gp.quicksum(
            gp.min_(0, water_traded_sub[i, j, h])  # Only include negative elements
            for i in range(n_s)
            for j in range(n_c)
            for h in range(remaining_tw))/n_s
                <= remaining_wr,
            ...
            m.addConstr((
                gp.quicksum(
                    ...
                    for i in range(n_s)
                    for j in range(n_c)
                    for h in range(remaining_tw))/ n_s) - remaining_wr == gp.quicksum(
                    water_traded_sub[i, j, h] 
                    for i in range(n_s)
                    for j in range(n_c)
                    for h in range(remaining_tw))/n_s
                    ,
                    name=f"c.{water_right_id}.water_traded_{c_i}(cm)",)
            c_i += 1
        ...
        while remaining_length >= time_window:
            m.addConstr(
                ...
                / n_s) + gp.quicksum(gp.min_(0, water_traded_sub[i, j, h]) 
                                     for i in range(n_s)
                                     for j in range(n_c)
            ...
            m.addConstr((
                gp.quicksum(
                   ...
                   for j in range(n_c)
                   for h in range(start_index, start_index + time_window)
               )/ n_s) - wr_depth == gp.quicksum(
                    water_traded_sub[i, j, h] 
                    for i in range(n_s)
                    for j in range(n_c)
                    for h in range(start_index, start_index + time_window))/ n_s
                    ,
                    name=f"c.{water_right_id}.water_traded_{c_i}(cm)",)                         
            c_i += 1
        ...
        if remaining_length > 0:
            ...
            m.addConstr(
                ...
                / n_s) + gp.quicksum(
                    gp.min_(0, water_traded_sub[i, j, h])
                    for i in range(n_s)
                    for j in range(n_c)
                    for h in range(start_index, n_h)) / n_s
                <= wr_tail,
                name=f"c.{water_right_id}.wr_{c_i}(cm)",
            )
            m.addConstr((
            ...
            m.addConstr((
                gp.quicksum(
                   ...
                   for j in range(n_c)
                    ...
                    water_traded_sub[i, j, 0] 
                    for i in range(n_s)
                    for j in range(n_c))/ n_s
                    ,
                    name=f"c.{water_right_id}.water_traded_{c_i}(cm)",)
        else:
    ...
    def finish_setup(self, display_summary=True):
        ...
        water_traded = vars_["water_traded"]
        water_traded_per_field = vars_["water_traded_per_field"]
        y = vars_["y"]
        y_y = vars_["y_y"]
        e = vars_["e"]
        trading_cost = vars_["trading_cost"]
        ...
        m.addConstr(water_traded == get_sum(fids, vars_, "water_traded"), name="c.water_traded(cm)")
        ...
        m.addConstr(
            water_traded_per_field == get_sum(fids, vars_, "water_traded") / n_f,
            name="c.water_traded_per_field(cm)",
        )
        profit = vars_["profit"]
        rev = vars_["rev"]
        cost_e = vars_["cost_e"]
        ...
        trading_cost = vars_["trading_cost"]
        m.addConstr((profit == (rev - cost_e - annual_cost - trading_cost) / n_f), name="c.profit")
    ...
    def solve(
        ...
        **kwargs : **kwargs
        ...
        if m.Status == 2 or m.Status == 9:
            ...
            for k, v in wrs_info.items():
                if v["remaining_wr"] is not None:
                    ...
                    if fids == "all":
                        ...
                        irr_sub = irr_sub / len(fids)
                        if fids == "all":
                            water_traded_sub = sols["water_traded_per_field"]  # (n_s, n_c, n_h)
                        else:
                            for i, fid in enumerate(fids):
                                if i == 0:
                                    water_traded_sub = sols[fid]["water_traded"].copy()
                                else:
                                    water_traded_sub += sols[fid]["water_traded"]
                            water_traded_sub = water_traded_sub / len(fids)
                    v["remaining_wr"] -= np.sum(irr_sub[:, :, 0]) + np.sum(water_traded_sub[:, :, 0])
            ...
            water_trades = list(sols["irr_depth"].mean(axis=0).sum(axis=0).round(2))
            decisions = {"water trades depths": water_trades}
            for fid in fids:
                sols_fid = sols[fid]
