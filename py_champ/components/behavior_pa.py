    ...
    **kwargs
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.profit = None
        self.yield_rate = None
        self.zero_irrigation_reason = None
        self.store_irr_depth = {}
    ...
    def load_settings(self, settings: dict):
        ...
        self.seniority_id=settings["seniority_id"]
    ...
    def run_simulation(self):
        ...
        ##### Simulate fields
        for fi, field in self.fields.items():
            self.store_irr_depth = dm_sols[fi]["irr_depth"][:,:,[0]] 
            self.store_irr_depth = np.sum(self.store_irr_depth)
            
            planned_irr_depth = dm_sols[fi]["irr_depth"][:,:,[0]]
            planned_vol = np.sum(planned_irr_depth * field.unit_area * 0.01)  # Convert from cm to m
    
            # Calculate the remaining volume before hitting the cutoff
            if current_year >= 2002:
                ...
                if planned_vol > remaining_volume:
                    if remaining_volume > 0:  # Partial cutoff
                        scale_factor = remaining_volume / planned_vol
            ...
            # Override irrigation reasoning for rainfed fields
            ...
            irr_vol_p_f = field.irr_vol_per_field
            self.model.accumulated_withdrawal += irr_vol_p_f
