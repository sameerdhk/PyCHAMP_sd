...
class Optimization:
    ...
    def setup_constr_finance(self, finance_dict):
        ...
        # pumping fee
        pumping_fee = finance_dict['pumping_fee']
        cost_p = m.addMVar((n_h), vtype="C", name="cost_p(1e4$)", lb=0, ub=inf)
        m.addConstr((cost_p == v * pumping_fee), name =  "c.cost_p") 
    ...
    def finish_setup(self, pf_start = None, display_summary=True):
        """
        ...
        if current_year >= pf_start:
            m.addConstr((profit == (rev - cost_e - cost_p - annual_cost)/n_f), name="c.profit")
        else:
