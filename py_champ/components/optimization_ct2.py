...
class Optimization:
    ...
    def setup_ini_model(
        ...
        # total water traded per split per crop per yr
    ...
    def setup_constr_field(
        ...
        **kwargs:
        ...
        water_traded = m.addMVar(
            (n_s, n_c, n_h), vtype="C", name=f"{fid}.water_traded(cm)", lb=-ub_irr, ub=ub_irr
        )
        ...
        if wr_dict["applied_field_ids"] == "all" or fid in wr_dict["applied_field_ids"]:
            wr_depth =  wr_dict["wr_depth"]           
            m.addConstr(water_traded == (irr_depth - wr_depth) * i_crop, name=f"c.{fid}.water_traded(cm)")
            # m.addConstr((water_traded == irr_depth - wr_depth), name=f"c.{fid}.water_traded(cm)")
            # m.addConstr((water_traded * (1 - i_crop) == 0), name=f"c.{fid}.water_traded_(cm)")
        cm2m = 0.01
        m.addConstr((v_c == irr_depth * unit_area * cm2m), name=f"c.{fid}.v_c(m-ha)")
        # m.addConstr((wt_c == water_traded * unit_area * cm2m), name=f"c.{fid}.wt(m-ha)") #water traded volume
        m.addConstr(
        ...
        self.vars_[fid]["water_traded"] = water_traded
    ...
    def setup_constr_finance(self, finance_dict):
        ...
        trading_cost = m.addMVar((n_h), vtype="C", name="trading_cost(1e4$)", lb=-inf, ub=inf)
        ...
        m.addConstr((trading_cost == water_traded * trading_price), name="c.trading_cost")
        m.addConstr(
            rev
            == gp.quicksum(
                y[i, j, :] * crop_profit[c]
    ...
    def solve(
        ...
        **kwargs : **kwargs
        ...
        if m.Status == 2 or m.Status == 9:
            ...
            trades = list(sols["water_traded"].mean(axis=0).sum(axis=0).round(2))
